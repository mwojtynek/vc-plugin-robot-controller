...
void frameToRPY(double * rpy, const KDL::Frame * frame)
{
	frame->M.GetRPY(rpy[3], rpy[4], rpy[5]);
	memcpy(rpy, frame->p.data, 3 * sizeof(double));
}

void RPYToFrame(KDL::Frame * frame, const double * rpy)
{
	*frame = KDL::Frame(KDL::Rotation::RPY(rpy[3], rpy[4], rpy[5]),
		KDL::Vector(rpy[0], rpy[1], rpy[2]));
}
